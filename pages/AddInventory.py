...
sh, _ = init_services()
setting_ws, set_df, ITEM_OPTIONS, BRAND_OPTIONS, STATUS_OPTIONS, display_worksheets = get_system_settings(sh)
        ...
        with st.form(key=f"form_{ws.id}", clear_on_submit=True):
            ...
            if submit:
                with st.spinner("雲端處理中..."):
                    ...
                    # 寫入新選項到設定表
                    needs_update = False
                    if final_item not in ITEM_OPTIONS: ITEM_OPTIONS.append(final_item); needs_update = True
                    if final_brand and final_brand not in BRAND_OPTIONS: BRAND_OPTIONS.append(final_brand); needs_update = True
                        
                    if needs_update:
                        mlen = max(len(ITEM_OPTIONS), len(BRAND_OPTIONS))
                        new_df = pd.DataFrame({"下拉選單_品項名稱": ITEM_OPTIONS + [""]*(mlen-len(ITEM_OPTIONS)), "下拉選單_品牌": BRAND_OPTIONS + [""]*(mlen-len(BRAND_OPTIONS))})
                        setting_ws.clear()
                    ...
                    for col in header:
                        ...
                        elif col == "品牌": row_to_add.append(final_brand)
